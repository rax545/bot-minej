...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addIntegerOption(option =>
            option.setName('seconds')
                .setDescription('How many seconds to skip ahead (default 10)')
                .setMinValue(1)
                .setMaxValue(3600)
                .setRequired(false)
        ),
    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            const target = position + (seconds * 1000);

            if (duration > 0 && target >= duration) {
                const embed = new EmbedBuilder().setDescription(`❌ That goes past the end of the song (${formatDuration(duration)}). Use \`/skip\` instead.`);
                return interaction.editReply({ embeds: [embed] })
                    .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));
            }
            ...
            const embed = new EmbedBuilder()
                .setDescription(`⏩ Forwarded **${seconds}s** → **${formatDuration(target)}**${duration > 0 ? ` / ${formatDuration(duration)}` : ''}`);
            return interaction.editReply({ embeds: [embed] })
                .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));

        } catch (error) {
            console.error('Forward command error:', error);
            const embed = new EmbedBuilder().setDescription('❌ An error occurred while forwarding the song!');
            return interaction.editReply({ embeds: [embed] })
                .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));
        }
    }
