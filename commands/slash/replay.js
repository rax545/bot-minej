...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            const title = player.current?.info?.title || 'Unknown Title';

            if (player.current?.info?.seekable === false || player.current?.info?.stream) {
                const embed = new EmbedBuilder().setDescription('❌ This track is a live stream and cannot be replayed!');
                return interaction.editReply({ embeds: [embed] })
                    .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));
            }
            ...
            const embed = new EmbedBuilder().setDescription(`🔄 Replaying from the start: **${title}**`);
            return interaction.editReply({ embeds: [embed] })
                .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));

        } catch (error) {
            console.error('Replay command error:', error);
            const embed = new EmbedBuilder().setDescription('❌ An error occurred while replaying the song!');
            return interaction.editReply({ embeds: [embed] })
                .then(() => setTimeout(() => interaction.deleteReply().catch(() => {}), 3000));
        }
    }
